...
print("The softmax action function was use for the output layer because it will convert the 10 outputs into probabilities that are not negative and that will sum to 1 for multi class classification") 
...
print("The size of the first hidden layer is (None, 300). None means the batch size is not known at build time .\
The total number of parameters of the first hidden layer is 235500, which refers to the number of trainable weights plus biases ") 
model.summary()
...
print("An epoch is a single iteration over the training set") 
